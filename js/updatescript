...
var prephrase = '<span class="red">visitor@sneakyevil.eu</span>:<span class="blue">~</span># ';
...
{
	var res = '';
	if (command === './welcome.sh') res = 'Welcome to <span class="red">sneakyevil.eu</span>!<br><br>You\'re visiting from: <span class="red">' + visitorCountry + '</span><br>Last 5 Country Visits: <span class="red">' + lastCountry0 + '</span>, <span class="red">' + lastCountry1 + '</span>, <span class="red">' + lastCountry2 + '</span>, <span class="red">' + lastCountry3 + '</span>, <span class="red">' + lastCountry4 + '</span>.<br>Total Visits: <span class="red">' + visits + '</span><br>Terminal Updated: <span class="red">' + lastUpdate +'</span><br><br><b>Type \'help\' for more details on the commands</b>'
	else
	{
...
$('form').on('submit', function(e) {
	...
	try {
			...
			if (vInput.startsWith("./")) {
				if (vInput === "./root/welcome.sh") {
					...
					launchCommandAsVisitor({
						cmd: vInput,
						res: '<div><p>sneakyevil.eu: ' + vInput + ': No such file or directory</p></div>'
					});
				}
				...
				if (typeof loginArray[2] !== "undefined") {
					if (loginArray[2] === "visitor") {
						...
						res = 'sneakyevil.eu: No passwd entry for user ' + loginArray[2] + '';
					}
					launchCommandAsVisitor({
				...
				if (vInput === "music play") {
					...
					launchCommandAsVisitor({
						cmd: vInput,
						res: '<div><p>[ <span class="red">sneakyevil.eu</span> ] Usage: music [play/stop/link]</p></div>'
					});
				}
				...
				if (vInput === "friend joffrey") {
					...
					launchCommandAsVisitor({
						cmd: vInput,
						res: '<div><p>[ <span class="red">sneakyevil.eu</span> ] Usage: friend [name/list]</p></div>'
					});
				}
		...
		if (root > 0) {
			launchCommandAsRoot({
				cmd: $('input').val(),
				res: '<div><p>sneakyevil.eu: ' + $('input').val() + ': command not found</p></div>'
			});
		} else {
			launchCommandAsVisitor({
				cmd: $('input').val(),
